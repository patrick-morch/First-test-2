...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .select, input[type="date"] { background:#0f131a; border:1px solid #263040; color:var(--text); padding:8px 10px; border-radius:10px; }
  ...
  <!-- Firebase (valgfritt, la stå om du vil synke på tvers av enheter) -->
...
<body>
  ...
  <script>
    // --- Konfig: Fyll inn for fler-enhets-synk (ellers brukes kun lokal lagring) ---
    const FIREBASE_CONFIG = {
      // TODO: legg inn dine nøkler fra Firebase Console.
      // apiKey: "",
      // authDomain: "",
      // projectId: "",
      // appId: "",
    ...
    // --- Global state ---
    let state = {
      fields: [],                   // [{id,name,type,goal,includeInProgress,axisHint,updatedAt}]
      entries: {},                  // { "YYYY-MM-DD": { [fieldId]: value }, ... }
      uid: null,
      online: false,
      axisMapMode: 'auto',
      selectedFields: []
    ...
    // --- Firebase (valgfritt) ---
    let fb = { app:null, auth:null, db:null, enabled:false };
    function tryInitFirebase() {
      try {
        ...
        console.warn('Firebase init feilet, fortsetter lokalt', e);
        fb.enabled = false;
      }
    }
    ...
    async function signIn() {
      ...
      try {
        // Google hvis mulig, ellers anonym.
        const provider = new firebase.auth.GoogleAuthProvider();
        const res = await fb.auth.signInWithPopup(provider).catch(async () => {
    ...
    function signOut() {
      if (fb.enabled && fb.auth.currentUser) fb.auth.signOut();
    }
    ...
    async function startSync(uid) {
      ...
      unsubFields = fb.db.collection('users').doc(uid).collection('fields')
        .onSnapshot(async snap => {
          ...
          state.fields = arr.sort((a,b)=>a.name.localeCompare(b.name));
      ...
      unsubEntries = fb.db.collection('users').doc(uid).collection('entries')
        .onSnapshot(async snap => {
          ...
          state.entries = all;
          for (const [date,values] of Object.entries(state.entries)) {
    ...
    // --- Import / eksport (kompatibel med din nåværende app) ---
    function exportJSON() {
    ...
    async function importJSON(file) {
      ...
      if (Array.isArray(data.fields)) {
        for (const f of data.fields) await pushField({ ...f, updatedAt: Date.now() });
      }
    ...
    // --- Utils: aggregering, glatting, akse ---
    function dateRange(period) {
      const now = new Date();
      if (period==='all') return null;
      const map = { '7d':7, '30d':30, '365d':365 };
      return addDays(now, -map[period]);
    }
    function groupKey(d, mode) {
      const x = new Date(d);
      if (mode==='week') {
        const day = (x.getUTCDay()+6)%7; // ISO uke starter mandag
        const monday = new Date(Date.UTC(x.getUTCFullYear(), x.getUTCMonth(), x.getUTCDate()-day));
    ...
    function movingAvg(arr, win) {
      ...
      for (const v of arr) {
        ...
        out.push( q.length===win ? sum/win : null );
      }
      return out;
    }
    function pickAxis(field) {
      if (state.axisMapMode==='manual' && field.axisHint) return field.axisHint;
      // auto: score/% -> left(100), tid -> right(10)
      const n = field.name.toLowerCase();
      if (n.includes('score') || n.includes('%')) return 'left';
      if (n.includes('time') || n.includes('timer') || n.includes('min')) return 'right';
      // fallback: tall med mål over 10 → left, ellers right
    ...
    // --- Render: Chart.js ---
    let chart;
    function buildDatasets(dates, fields, matrix, smoothMode) {
      const colors = [
        '#5aa7ff','#7dd3fc','#a78bfa','#f472b6','#fca5a5','#f59e0b','#34d399','#4ade80','#22d3ee','#60a5fa','#93c5fd'
      ];
      const ds = [];
      fields.forEach((f, idx) => {
        const serie = dates.map(d => (matrix[d] && typeof matrix[d][f.id] !== 'undefined') ? Number(matrix[d][f.id]) : null);
        const yAxisID = pickAxis(f)==='left' ? 'y' : 'y1';
        const data = smoothMode==='ma7' ? movingAvg(serie,7)
                  : smoothMode==='ma28' ? movingAvg(serie,28)
                  : serie;
        ds.push({
          label: f.name,
          data,
        ...
        if (typeof f.goal === 'number') {
          ds.push({
            label: f.name + ' mål',
            data: dates.map(()=>f.goal),
            borderColor: colors[idx % colors.length],
            borderDash: [6,6],
            pointRadius: 0,
            tension: 0,
    ...
    function renderChart() {
      ...
      const datasets = buildDatasets(labels, fields, matrix, smooth);
      ...
      // enkel legend-toggle (Chart.js har innebygd, men vi vil synkronisere feltlisten)
    ...
    // --- Render: Heatmap, KPIs, Trender ---
    function renderHeatmap() {
      const host = document.getElementById('heatmap');
      host.innerHTML = '';
      // 12 uker tilbake, hver kolonne = uke, celle = dag
      const end = new Date(); end.setUTCHours(0,0,0,0);
      const start = addDays(end, -7*12);
      const days = [];
      for (let d = new Date(start); d <= end; d = addDays(d, 1)) days.push(iso(d));
      const weeks = [];
      for (let i=0; i<12; i++) weeks.push(days.slice(i*7, i*7+7));
      weeks.forEach(week => {
    ...
    function renderKPIs() {
      ...
      const counts = (n) => {
        ...
        return dates.filter(d => new Date(d) >= since && Object.keys(state.entries[d]).length>0).length;
      };
      // streak
      let streak = 0;
      for (let d=new Date(now); ; d=addDays(d,-1)) {
        const s = iso(d);
        if (state.entries[s] && Object.keys(state.entries[s]).length>0) streak++;
        else break;
      }
      // % fullført i dag
      const today = todayISO();
      const todayVals = state.entries[today] || {};
      const maxCount = state.fields.filter(f=>f.includeInProgress && f.type!=='checkbox').length; // ex: velg selv
    ...
    function renderTrends() {
      ...
      const target = sel.length===1 ? state.fields.find(f=>f.id===sel[0]) : null;
      const t7 = document.getElementById('trend7');
      const t28 = document.getElementById('trend28');
      const hint = document.getElementById('trendHint');
      if (!target) {
        t7.textContent = '0'; t28.textContent='0'; hint.style.display='block'; return;
      }
      hint.style.display='none';
      // delta mellom siste verdi og snitt siste 7/28 dager
      const dates = Object.keys(state.entries).sort();
      ...
      const avg28 = avg(values.slice(-28));
      const f = (x) => (x>=0?'+':'') + x.toFixed(2);
      t7.textContent = f(last-avg7);
      t28.textContent = f(last-avg28);
    }
    ...
    function renderFieldSelect() {
      ...
      for (const id of state.selectedFields) {
        const o = [...sel.options].find(o=>o.value===id);
        if (o) o.selected = true;
      }
      sel.onchange = () => {
        state.selectedFields = [...sel.selectedOptions].map(o=>o.value);
        renderAll();
      };
    }
    ...
    // --- Boot: last fra IndexedDB, start Firebase hvis mulig ---
    async function boot() {
      await idbOpen();
      state.fields = await idbGetAll('fields');
      ...
      document.getElementById('grouping').onchange =
      document.getElementById('agg').onchange =
      document.getElementById('smooth').onchange = () => renderAll();
      ...
      if (fb.enabled) {
        ...
        firebase.auth().onAuthStateChanged(async (user) => {
          if (user) {
            state.uid = user.uid;
            syncStatus.textContent = 'Synk på: ' + (user.displayName || 'konto');
    ...
    // --- Demo: hvis ingen data, lag noen eksempel-felt så du ser UI ---
    async function ensureDemo() {
      ...
      const demoFields = [
        ...
        { id:'steps', name:'Skritt', type:'number', goal:10000, includeInProgress:false, axisHint:'left', updatedAt:Date.now() },
      ];
      ...
      for (let i=0;i<21;i++){
        ...
        await pushEntry(d, {
          sleep_score: Math.max(50, Math.min(100, Math.round(80 + (Math.random()*20-10)))),
          sleep_hours: Math.max(4, Math.min(10, Math.round(7 + (Math.random()*4-2)))),
          steps: Math.round(8000 + (Math.random()*8000-4000))
        });
      }
    }
